// File Path: src/pages/volunteer/ViewProblemsVolunteer.jsx
...
const ViewProblemsVolunteer = () => {
  // Static data for demonstration purposes
  const problems1 = [
    {
      id: 1,
      title: 'Problem A',
      description: 'This is a description of Problem A.',
      replies: [],
    },
    {
      id: 2,
      title: 'Problem B',
      description: 'This is a description of Problem B.',
      replies: [],
    },
    {
      id: 3,
      title: 'Problem C',
      description: 'This is a description of Problem C.',
      replies: [],
    },
  ];

  
  const handleReplySubmit = (problemId, reply) => {
    const updatedProblems = problemList.map(problem => {
      if (problem.id === problemId) {
        return {
          ...problem,
          replies: [...problem.replies, reply],
        };
      }
      return problem;
    });
    setProblemList(updatedProblems);
    setShowReplyForm({ ...showReplyForm, [problemId]: false });
  };
  ...
  const handleTextChange=(e)=>{
   ...
   const sendReply=async(id)=>{
    try {
      const response = await AxiosApi.post(`problem/reply/${id}/?reply=${text}` )
                  ...
                  <li>
                    <details className="group">
                      ...
                      <article className="px-4 pb-4">
                        ...
                        <p className="text-gray-700 ">
                         <textarea name="" id=""  className='border rounded-lg w-full p-3 m-3' onChange={handleTextChange}></textarea>
                        </p>
                                       <button onClick={()=>sendReply(item.id)} className="rounded-lg hover:text-gray-600 p-3 w-16 m-10 bg-green-700 text-white">Reply</button>
...
export default ViewProblemsVolunteer;
